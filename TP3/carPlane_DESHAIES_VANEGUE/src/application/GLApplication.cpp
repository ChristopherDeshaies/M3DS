...
void GLApplication::updateCamera() {
  ...
  Quaternion tmp = _car.orientation(); //q17
  Quaternion tmp2 = _airplane.orientation(); //q17

  tmp.setRotation(270, Vector3(0,1,0)); //q17
  tmp2.setRotation(180, Vector3(0,1,0)); //q17

  Vector3 airplanePos = _airplane.position() + _airplane.orientation() * Vector3(0, 1, -4); //q17
  Vector3 carPos = _car.position() + _car.orientation() * Vector3(-7, 2, 0); //q17
  Quaternion airplaneOrien =_airplane.orientation()*tmp2; //q17
  Quaternion carOrien = _car.orientation()*tmp; //q17
  ...
  switch (_cameraMode) {
    ...
    case Camera_Follow_Car: {
      //q16
      _cameraStop.position(carPos);
      _cameraStop.orientation(carOrien);

     // if(_cameraStop != _camera)){
        _camera.position(  ( 1 - _lambda ) * airplanePos + _lambda * carPos );
        _camera.orientation( ( 1 - _lambda ) *  airplaneOrien + _lambda * carOrien);
     // }
    ...
    case Camera_Follow_Plane: {
      //q15
    //  _cameraStop.position(_airplane.position() + _airplane.orientation() * Vector3(0, 1, -4));
    //  _cameraStop.orientation(_airplane.orientation()*tmp2);

      _cameraStop.position(airplanePos);
      _cameraStop.orientation(airplaneOrien);

      _camera.position( ( 1 - _lambda ) * carPos +  _lambda * airplanePos);
      _camera.orientation( ( 1 - _lambda ) * carOrien + _lambda * airplaneOrien);
