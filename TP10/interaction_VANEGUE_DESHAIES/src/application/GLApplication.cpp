    ...
    if (mouseRight()) {
        if (_selectedIntersection) { // an intersection has been found (in GLApplication::selectObject() ).
            ...
            if (_controlMouse==Manipulation_Translation) {
                ...
                Vector3 transform = _camera.directionTo(Coordinate_World, Vector3(dx, dy, 0));
                Vector3 translation = mesh->directionTo(Coordinate_Local, transform);
                mesh->translate(translation, Coordinate_Local);
            }
...
void GLApplication::updateCamera() {
    ...
    double facteurDeplacement = 0.5;

    if (left()) {
        _camera.translate(-facteurDeplacement, 0, 0, Coordinate_Local);
    }
    if (right()) {
        _camera.translate(facteurDeplacement, 0, 0, Coordinate_Local);
    }
    if (forward()) {
        _camera.translate(0, 0, -facteurDeplacement, Coordinate_Local);
    }
    if (backward()) {
        _camera.translate(0, 0, facteurDeplacement, Coordinate_Local);
    }
    if (mouseLeft()) {
        ...
        //méthode fonctionnel avec tangage
        //_camera.rotate(-deltaMouseX(), 0, 1, 0, Coordinate_Local);
        //_camera.rotate(deltaMouseY(), 1, 0, 0, Coordinate_Local);

        //solution sans tangage
        Vector3 axeX = _camera.directionTo(Coordinate_Local, Vector3(0,-1,0));
        _camera.rotate(deltaMouseX(), axeX, Coordinate_Local);
        _camera.rotate(deltaMouseY(), 1, 0, 0, Coordinate_Local);
